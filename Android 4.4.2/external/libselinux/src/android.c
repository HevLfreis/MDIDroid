				...
				context_t ctx)
{
	...
	int userapp = 0;
	if (seinfo!=NULL) {
        	if (!((0==strcasecmp(seinfo, "platform"))||(0==strcasecmp(seinfo, "media"))
              ||(0==strcasecmp(seinfo, "shared"))||(0==strcasecmp(seinfo, "release")))) {
            	seinfo = pkgname;
				userapp = 1;
				selinux_log(SELINUX_INFO, \
					"this app a user app: %s ----seinfo : %s", pkgname, seinfo);
        	}
	...
	for (i = 0; i < nspec; i++) {
		...
		// Modify by XJTU, solve problem by sub process
		if (cur->seinfo) {
			if (!seinfo || strcasecmp(seinfo, cur->seinfo))
				if (userapp) {
					if (strncmp(seinfo, cur->seinfo, strlen(cur->seinfo)))
						continue;
				}
				else {
					continue;
				...
				uid_t uid)
{
	...
	//Modify by XJTU , force reload seapp_contexts
			       ...
			       const char *pkgname)
{
	...
	if (strcmp(ctx_str, orig_ctx_str)) {
		selinux_log(SELINUX_INFO, \
			"%s belongs to : %s ", pkgname, ctx_str);
		rc = setcon(ctx_str);
		if (rc < 0) {
			selinux_log(SELINUX_INFO, \
					"setcon error");
